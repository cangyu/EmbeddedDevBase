...
static int32_t Receive_Packet (uint8_t *data, int32_t *length, uint32_t timeout)
{
  ...
  *data = c;//接受到的第一个字节赋给data的首位
  for (i = 1; i < (packet_size + PACKET_OVERHEAD); i ++)//接受数据
  {
	...
	//根据YMODEM协议，第三个字节是第二个字节（序列号）的反码
	//检查是否满足
      ...
      switch (Receive_Packet(packet_data, &packet_length, NAK_TIMEOUT))
      {
        case 0:
          ...
          switch (packet_length)
          {
            ...
            /* Normal packet */
            default:
              if ((packet_data[PACKET_SEQNO_INDEX] & 0xff) != (packets_received & 0xff))//判断序列号是否正确
              {
                Send_Byte(NAK);
              }
              else
              {
                ...
                {
                  memcpy(buf_ptr, packet_data + PACKET_HEADER, packet_length);//将收到的信息字节（从第四个字节开始）转移到buf_ptr中
                  RamSource = (uint32_t)buf;//buf
